...
namespace ProgrammingAssignment1
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            float deltaX, deltaY, deltaXPow, deltaYPow, distance, radians, angle;
            ...
            while (input[0] != 'q')
            {
                ...
                deltaXPow = (float) Math.Pow(deltaX, 2);
                deltaYPow = (float)Math.Pow(deltaY, 2);
                distance = (float)Math.Pow(deltaXPow + deltaYPow, 0.5);
                radians = (float)Math.Atan2(deltaY, deltaX);
                angle = radians * (180 / (float)Math.PI);
